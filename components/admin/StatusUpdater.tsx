...
export function StatusUpdater({ quoteId, current, notes }: { quoteId: string; current: string; notes: string | null }) {
  ...
  return (
    <div className="space-y-4">
      <div>
        <label className="block text-xs font-semibold text-stone-500 uppercase tracking-wider mb-2">Estado</label>
        <select
          value={status}
      ...
      <div>
        <label className="block text-xs font-semibold text-stone-500 uppercase tracking-wider mb-2">Notas internas</label>
        <textarea
          value={adminNotes}
      ...
      >
        {saving ? <><Loader2 className="w-4 h-4 animate-spin" /> Guardando...</> : saved ? "✓ Guardado" : "Guardar cambios"}
      </button>
    </div>
  )
}
